...
const PickupList = () => {
  ...
  const redirectToPickupDetails = (pId) => {
    navigate(`/profile/pickupitems/${pId}`);
  };
if (!user) {
    return <p>Loading...</p>;
  }
    ...
    <div key={pickup.id} className="bg-white rounded-lg shadow-md mb-8 p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-gray-100 p-4 rounded-lg">
          <p className="text-green-700"><span className="font-extrabold">Pickup Type:</span> {pickup.pickup_type}</p>
          <p><span className="font-extrabold">Pickup Date:</span> {pickup.pickup_date}</p>
          <p>
          <span className="font-extrabold"> Address: </span>{pickup.pickup_address.address1}, {pickup.pickup_address.city.name}, {pickup.pickup_address.pincode}, {pickup.pickup_address.city.phone1}
          </p>
        </div>
        <div className="bg-gray-100 p-4 rounded-lg">
          <p> <span className="font-extrabold"> Weight: </span><span className="font-semibold text-red-700">{pickup.weight}Kg</span></p>
          <p><span className="font-extrabold">Price: </span> <span className="font-semibold text-purple-700">{pickup.price}Rs</span></p>
          <p><span className="font-extrabold">Status: </span><button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded mt-2">
            {pickup.pickup_status}
          </button></p>
        </div>
        <div className="bg-gray-100 p-4 rounded-lg">
          <p> <span className="font-extrabold"> Notes:</span> {pickup.special_instructions}</p>
        </div>
      </div>
